...
import TreeNode from './TreeNodeModel';
...
function Admin({redirect}) {    
  ...
  const [simpleOperations, setsimpleOperations] = useState([]);
   ...
   function ModalOperation ({game}) {
      ...
      <div className="modal-overlay">
        <div className="modal">
          ...
          <div className="long-scrollable-list" id="scrollable-list" style={{marginRight:"20px"}}>
              <div
                className={`list-item`} style={{backgroundColor:"#e8f8ff", fontWeight:"bold", fontSize:"30px", display:"flex"}}>
                v.1.0.0
              </div>
            </div>
        <div style={{margin:"auto"}}>
          <div className="date-field">
            <span>HandVelocityX</span>
            <select id="dropdown" className="raw-data-input">
              <option value="">Ninguno</option>
            </select>
          </div>
          <br></br>
          <div className="date-field">
            <span>HandVelocityY</span>
            <select id="dropdown" className="raw-data-input">
              <option value="">Ninguno</option>
            </select>
          </div>
          ...
          <button className="button-patients-list" onClick={handleCloseModal}>Actualizar</button>
    ...
    const handleOpenModal = (operationName) => {
      setIsModalOpen(true);
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try{
        ...
        console.log(newVersionActivated);
      }catch(error){
        console.log(error);
      }
    }
  ...
  const activateNewOperationPanel = gameId => () =>{
    ...
    console.log(operationActivatedTemp)
  }
  ...
  const addVersion = (gameId, name) => async () => {
    ...
    if(responseVersion.ok){
      ...
      if(responseParameters.ok){
        ...
        parameters.push(responseParametersParsed.map(p=>({id:responseVersionParsed,name:p.name})))
  ...
  <>
  <div class="app">
    {isModalOpen && (<Modal operationName={selectedOperation}/>)}
    ...
    <div className="admin-container">
        ...
        <div className="list-container admin-panel">
          ...
          {operation.filter(Operation=>Operation.id===game.id)[0]?.data.map((Operation, index) => (
            <div onClick={()=>handleOpenModal(Operation.name)}
              key={index}
              className={`list-item`}>
              {Operation.name}
            </div>
